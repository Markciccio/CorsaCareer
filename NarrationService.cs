...
public static class NarrationService
{
    ...
    public static bool PlayAudioFile(string path)
    {
        ...
        Stop();
        // Il player integrato di Windows è la prima scelta per l'MP3 incluso.
        // La build distribuita non richiede quindi ffplay installato a parte.
        var alias = "corsacareer_voice";
        var escaped = path.Replace("\"", "\"\"");
        if (SoundtrackService.TryPlayOneShot(escaped, alias)) return true;
    ...
    private static void OpenGuidedStory(IReadOnlyList<BrowserPortalContent> stories, CareerState career)
    {
        ...
        var chapters = stories.Select((story, index) => new
        {
            ...
            recorded = index == 0 ? ToDataUri(AssetPaths.File("narration", "chapter1-intro-narration.mp3"), "audio/mpeg") : "",
        ...
        // La copertina resta ferma: il lettore scorre soltanto il testo.
        // L'audio registrato è attivabile direttamente dalla tavola, senza
        // costringere a raggiungere il vecchio controllo in fondo all'articolo.
        html = html
            .Replace(".page{min-height:690px;overflow:hidden}", ".page{height:calc(100vh - 185px);min-height:620px;overflow:hidden;display:flex;flex-direction:column}")
            .Replace(".cover{height:310px;background:#080b10;overflow:hidden}", ".cover{height:310px;flex:none;background:#080b10;overflow:hidden;position:relative}.reader{min-height:0;overflow-y:auto}.cover-audio{position:absolute;left:22px;bottom:20px;border:0;border-radius:3px;background:#e71946;color:#fff;padding:12px 17px;font-weight:800;cursor:pointer;box-shadow:0 3px 15px #0009}.cover-audio:hover{background:#ff3158}")
            .Replace("<section class='page'><div class='cover'><img id='art' alt='Illustrazione narrativa del capitolo'></div><div class='hero'>", "<section class='page'><div class='cover'><img id='art' alt='Illustrazione narrativa del capitolo'><button class='cover-audio' onclick='playNarration()'>▶ ASCOLTA LA PREFAZIONE</button></div><div class='reader' id='reader'><div class='hero'>")
            .Replace("</div></section></main>", "</div></div></section></main>")
            .Replace("onclick='speak()'>▶ ASCOLTA</button><button onclick='speechSynthesis.cancel()'>■ FERMA", "onclick='playNarration()'>▶ ASCOLTA</button><button onclick='stopNarration()'>■ FERMA")
            .Replace("q('art').src=c.art;", "q('art').src=c.art;q('reader').scrollTop=0;")
            .Replace("scrollTo({top:0,behavior:'smooth'});speechSynthesis.cancel()", "stopNarration()")
            .Replace("function speak(){speechSynthesis.cancel();", "function stopNarration(){const a=q('recorded');a.pause();a.currentTime=0;speechSynthesis.cancel()}function playNarration(){const a=q('recorded'),c=chapters[current];stopNarration();if(c.recorded){a.src=c.recorded;a.play().catch(speak);return}speak()}function speak(){speechSynthesis.cancel();")
            .Replace("<script>", "<audio id='recorded' preload='metadata'></audio><script>");
        File.WriteAllText(path, html, Encoding.UTF8);
        Process.Start(new ProcessStartInfo { FileName = path, UseShellExecute = true });
    ...
    private static string ToDataUri(string path, string mimeType = "image/png")
    {
        try { return File.Exists(path) ? $"data:{mimeType};base64," + Convert.ToBase64String(File.ReadAllBytes(path)) : ""; }
        catch { return ""; }
    }
    ...
    public static void Stop()
    {
        lock (Gate)
        {
            SoundtrackService.StopOneShot("corsacareer_voice");
